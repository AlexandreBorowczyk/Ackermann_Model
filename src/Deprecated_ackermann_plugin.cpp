...
void AckermannPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("wheelTrack"))
    ...
    wheelTrack = _sdf->GetElement("wheelTrack")->Get<double>();

  if (!_sdf->HasElement("wheelBaseLength"))
    wheelBaseLength = 0.32;
  else
    wheelBaseLength = _sdf->GetElement("wheelBaseLength")->Get<double>();
...
*/
  ...
  ros::SubscribeOptions so =
      ros::SubscribeOptions::create<gazebo_f150::DriveCmd>(topicName, 1,
...
void AckermannPlugin::Update()
{
  ...
  if (enableMotors)
  {
    joints[LEFT]->SetVelocity(2, wheelSpeed[LEFT] / (wheelDiam / 2.0));

    joints[RIGHT]->SetVelocity(2, wheelSpeed[RIGHT] / (wheelDiam / 2.0));
...
void AckermannPlugin::carDriveCallback(const gazebo_f150::DriveCmd::ConstPtr& cmd_msg)
{

  lock.lock();

  speed_ = cmd_msg->speed;
  turnRadius_ = cmd_msg->turn_radius;
